<script lang="ts">
	import TypingLinesEditor from '$lib/components/admin/TypingLinesEditor.svelte';
	...
	$effect.pre(() => {
		data.lesson.id;
		lines = cloneLines(data.lesson.steps);
	});
</script>
...
<div class="admin-editor">
	<header class="admin-page-head">
		...
		<p class="sub">Typing lesson · {lines.length} lines · ID {data.lesson.id}</p>
	</header>
	...
	<form
		...
		onsubmit={(e) => {
			const input = e.currentTarget.querySelector('input[name=lines]') as HTMLInputElement;
	...
	>
		...
		<TypingLinesEditor lessonId={data.lesson.id} bind:lines />
		<input type="hidden" name="lines" value="" />

		<div class="admin-sticky-bar">
			<span class="bar-hint">{lines.length} line{lines.length === 1 ? '' : 's'}</span>
